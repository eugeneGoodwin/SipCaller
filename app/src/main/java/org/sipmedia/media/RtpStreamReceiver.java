...
public class RtpStreamReceiver extends Thread {
	...
	public RtpStreamReceiver(SipSocket socket, Codecs.Map payload_type) {
		init(socket);
		p_type = payload_type;
	}

	/** Inits the RtpStreamReceiver */
	private void init(SipSocket socket) {
		if (socket != null)
	...
	void saveVolume() {
		if (restored)
			Receiver.setIntValue("volume"+speakermode, am.getStreamVolume(stream()));
	}
	
	void saveSettings() {
		if (!Receiver.getOldValid()) {
			...
			if (!Receiver.containsValueByKey(Receiver.PREF_OLDVIBRATE2))
			...
			map.put(Receiver.PREF_OLDVIBRATE, oldvibrate);
			map.put(Receiver.PREF_OLDVIBRATE2, oldvibrate2);
			map.put(Receiver.PREF_OLDPOLICY, oldpolicy);
			map.put(Receiver.PREF_OLDRING, am.getStreamVolume(AudioManager.STREAM_RING));
			Receiver.setMapIntValue(map);
			Receiver.setBooleanValue(Receiver.PREF_OLDVALID, true);
		}
	}
	...
	public static void setMode(int mode) {
		Receiver.setBooleanValue(Receiver.PREF_SETMODE, true);
		AudioManager am = (AudioManager) Receiver.mContext.getSystemService(Context.AUDIO_SERVICE);
		if (Integer.parseInt(Build.VERSION.SDK) >= 5)
	...
	public static void restoreMode() {
		if (Receiver.getSetMode()) {
			Receiver.setBooleanValue(Receiver.PREF_SETMODE, false);
	...
	void initMode() {
		if (Receiver.call_state == UA.UA_STATE_INCOMING_CALL &&
	...
	public static void restoreSettings() {
		if (Receiver.getOldValid()) {
			...
			int oldvibrate = Receiver.getOldVibrate();
			int oldvibrate2 = Receiver.getOldVibrate2();
			int oldpolicy = Receiver.getOldPolicy();
			...
			am.setStreamVolume(AudioManager.STREAM_RING, Receiver.getOldRing(), 0);
			Receiver.setBooleanValue(Receiver.PREF_OLDVALID, false);
			PowerManager pm = (PowerManager) Receiver.mContext.getSystemService(Context.POWER_SERVICE);
			PowerManager.WakeLock wl = pm.newWakeLock(PowerManager.SCREEN_BRIGHT_WAKE_LOCK |
					PowerManager.ACQUIRE_CAUSES_WAKEUP, "Sipmedia.RtpStreamReceiver");
			wl.acquire(1000);
		}
		restoreMode();
	}
	...
	public void run() {
		try {
			boolean nodata = Receiver.getNodata();

			if (rtp_socket == null) {
				return;
			}

			android.util.Log.d("RtpStreamReceiver", "start thread");
			...
			ToneGenerator tg = new ToneGenerator(AudioManager.STREAM_VOICE_CALL, (int) (ToneGenerator.MAX_VOLUME * 2 * Receiver.getEarGain()));
			track.play();
			System.gc();
			android.util.Log.d("RtpStreamReceiver", "start loop");
			while (running) {
				//Receiver.lock(true);
				if (Receiver.call_state == UA.UA_STATE_HOLD) {
					tg.stopTone();
					track.pause();
					while (running && Receiver.call_state == UA.UA_STATE_HOLD) {
						try {
							sleep(1000);
				...
				try {
					android.util.Log.d("RtpStreamReceiver", "start receive");
					rtp_socket.receive(rtp_packet);
					android.util.Log.d("RtpStreamReceiver", "end receive");
					if (timeout != 0) {
						tg.stopTone();
						track.pause();
				...
				if (running && timeout == 0) {
					...
					if (user >= luser + 8000 * mu && (
							Receiver.call_state == UA.UA_STATE_INCALL ||
									Receiver.call_state == UA.UA_STATE_OUTGOING_CALL)) {
